...
if( ( $is_free != 'yes' ) && empty( $payment_methods ) ) {
	...
	<div id="wcfmu-feature-missing-message" class="wcfm-warn-message wcfm-wcfmu" style="display: block;">
		...
		<?php esc_html_e( 'Something Wrong: You are not allowed to access this page now. Please contact Store Admin for more details.', 'wc-multivendor-membership' ); ?></p>
	</div>
	<?php
	return;
}

$is_wc_checkout = ( isset( $subscription['subscription_pay_mode'] ) && "by_wc" == $subscription['subscription_pay_mode'] ) ? true : false;

if ( $is_wc_checkout ) {
	?>
	<div id="wcfmu-feature-missing-message" class="wcfm-warn-message wcfm-wcfmu" style="display: block;">
		<p><span class="wcfmfa fa-warning"></span>
		<?php echo sprintf( __( 'Are you lost? <a href="%s">Continue to Payment</a>', 'wc-multivendor-membership' ), wc_get_checkout_url() ); ?></p>
	</div>
	<?php
	return;
}
...
<div id="wcfm_membership_container">
  <div class="wcfm_membership_review_pay">
		<div class="wcfm_membership_review_plan">
			<div class="wcfm_review_plan_welcome"><?php esc_html_e( 'Review your plan:', 'wc-multivendor-membership' ); ?></div>
			<div class="wcfm_review_plan_title"><?php esc_html_e( $title, 'wc-multivendor-membership' ); ?></div>
			<div class="wcfm_review_plan_description"><?php esc_html_e( $description, 'wc-multivendor-membership' ); ?>////////////</div>
			 ...
			 if( !empty( $membership_feature_lists ) ) {
					foreach( $membership_feature_lists as $membership_feature_key => $membership_feature_list ) {
						if( isset( $membership_feature_list['feature'] ) && !empty( $membership_feature_list['feature'] ) ) {
							...
							?>
							<div class="wcfm_review_plan_feature"><?php echo wp_kses_post(wcfm_removeslashes( __( $membership_feature_list['feature'], 'WCfM' ) )); ?></div>
							<div class="wcfm_review_plan_feature_val"><?php echo wp_kses_post(wcfm_removeslashes( __( $feature_val, 'WCfM' ) )); ?></div>
							<?php
						}
					}
			 }
		...
		<div class="wcfm_membership_pay">
			...
			if( $is_free == 'yes' ) {
				?>
				<div class="wcfm_review_pay_welcome"><?php esc_html_e( 'Confirmation', 'wc-multivendor-membership' ); ?>:</div>
				<form id="wcfm_membership_payment_form_free" class="wcfm wcfm_membership_payment_form">
					<div class="wcfm_review_pay_free"><?php echo wp_kses_post(apply_filters( 'wcfm_free_plan_payment_message', __( 'FREE Plan!!! There is no payment option for this.', 'wc-multivendor-membership' ) )); ?></div>
					<input type="hidden" name="member_id" value="<?php echo esc_attr($member_id); ?>" />
					<div class="wcfm-clearfix"></div>
					<div class="wcfm-message" tabindex="-1"></div>
					
					<div id="wcfm_membership_payment_submit" class="wcfm_form_simple_submit_wrapper">
						<input type="submit" name="save-data" value="<?php esc_html_e( 'Proceed', 'wc-multivendor-membership' ); ?>" id="wcfm_membership_payment_button_free" class="wcfm_membership_payment_button wcfm_submit_button" />
					</div>
					<div class="wcfm-clearfix"></div>
				</form>
					<?php
					...
					?>
					<div class="wcfm_review_pay_welcome"><?php esc_html_e( 'Payment options:', 'wc-multivendor-membership' ); ?></div>
					<?php
					...
					foreach( $payment_methods as $payment_method ) {
						...
						echo '<div class="wcfm_review_pay_non_free wcfm_review_pay_'.esc_attr($payment_method).'"><input class="wcfm_subscription_paymode" name="wcfm_subscription_paymode" type="radio" value="'.esc_html($payment_method).'" />' . wp_kses_post($wcfm_membership_payment_methods[$payment_method]) .' - '; 
						if( $subscription_type == 'one_time' ) {
							...
							echo '<span class="wcfm_membership_price_description"> (' . wp_kses_post($price_description) . ')</span>';
						}
						echo "</div>";
