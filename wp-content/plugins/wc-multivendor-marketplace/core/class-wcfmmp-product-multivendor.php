  	...
  	if( !wcfm_is_store_page() && apply_filters( 'wcfm_is_allow_product_loop_duplicate_hide', true ) ) {
			...
			if( !empty( $more_offers ) ) {
				foreach ($more_offers as $key => $value) {
					...
					$sql = "SELECT product_id, stock_status, stock_quantity FROM {$wpdb->wc_product_meta_lookup} wc_product_meta_lookup WHERE product_id IN (%s)  ORDER BY wc_product_meta_lookup.min_price ASC";
