...
trait Cost_Trait
{
     ...
     * @return array
     */
    public function calculate_rental_cost($productId, $inventoryId, $durations, $args = [])
    {
        ...
        $discount    = $this->calculate_discount($durationTotal, $hasDiscount);
        ...
        $pre_payment_total = get_option('rnb_instance_payment');
        if (empty($pre_payment_total) || $displays['instance_payment'] === 'closed') {
            $pre_payment_total = 100;
        ...
        if('percent' == $instance_payment_type){
            $instance_payment = ($cost * $pre_payment_total) / 100;
        }else{
            $instance_payment = $pre_payment_total;
        }
     ...
     * @return float
     */
    public function calculate_discount($durationTotal, $hasDiscount)
    {
        $discount = 0;
        ...
        if ($hasDiscount['discount_type'] === 'percentage') {
            ...
            $discountAmount = (float) $hasDiscount['discount_amount'];
        
            // Verificar si la conversión fue exitosa antes de realizar la operación
            if (is_float($discountAmount)) {
                $discount = ($durationTotal * $discountAmount) / 100;
            } else {
                // Manejar el caso en el que la conversión no fue exitosa
                //echo "Error: El descuento no es un número válido.";
            }
        }
