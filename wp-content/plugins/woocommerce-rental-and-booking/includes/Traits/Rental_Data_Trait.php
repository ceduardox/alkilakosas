...
trait Rental_Data_Trait
{
    ...
    {
        if (!empty($data['rental_days_and_costs']['instant_pay'])) {
            $instance_payment_type = rnb_get_instance_payment_type();
            $instant_pay = $data['rental_days_and_costs']['instant_pay'];
            $general = $settings['general'];
            $key   = $general['instant_pay_amount'] ? $general['instant_pay_amount'] : esc_html__('Instant Pay Amount', 'redq-rental');

            if ($instant_pay == 100) return [];
            $this->instant_pay = (float) $instant_pay;

            $results = [
                'settings_key' => ('percent' == $instance_payment_type) ? 'instant_pay_amount' : '',
                'instance_payment_type' => $instance_payment_type,
                'key'          => $key,
                'summary'      => true,
                'summary_key'  => $key,
                'type'         => 'single',
                ...
                'data'         => [
                    'name' => $key,
                    'cost' => $instant_pay . (('percent' == $instance_payment_type) ? '%' : ''),
                ],
            ];
    ...
    public function prepare_duration_data($data, $settings, $quantity = 1)
    {
        ...
        if ($breakdown['days']) {
            $duration .= $breakdown['days'] > 1 ? $breakdown['days'] . __(' days ', 'redq-rental') : $breakdown['days'] . __(' day ', 'redq-rental');
        }

        if ($breakdown['hours']) {
            $duration .= $breakdown['hours'] > 1 ? $breakdown['hours'] . __(' hours ', 'redq-rental') : $breakdown['hours'] . __(' hour ', 'redq-rental');
        }
    ...
    public function prepare_discount_total_data($data, $settings)
    {
        ...
        $general = $settings['general'];
        $key = $general['discount_amount'] ? $general['discount_amount'] : esc_html__('Total', 'redq-rental');
    ...
    public function prepare_total_data($data, $settings, $quantity = 1)
    {
        ...
        if ($this->instant_pay) {
            $deposit_total = isset($price_summary['deposit_total']) ? $price_summary['deposit_total'] : 0;
            $instant_pay   = $this->instant_pay;
            if ($instant_pay != 100) {
                $deposit_free_total = isset($price_summary['deposit_free_total']) ? $price_summary['deposit_free_total'] : 0;
                $deposit_free_total = (float) $deposit_free_total;
                if ('percent' == $instance_payment_type) {
                    $instant_pay_amount = ($instant_pay * $deposit_free_total) / 100;
                } else {
                    $instant_pay_amount = $instant_pay;
                }
                $total  = $deposit_total + $instant_pay_amount;
            }
            $total = apply_filters('rnb_instant_pay_total', $total, $instance_payment_type);
        }
