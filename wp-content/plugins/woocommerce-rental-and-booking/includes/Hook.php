...
class Hook
{
    ...
    public function prepare_price_summary($item_data, $product_id, $posted_data)
    {
        ...
        foreach ($item_data as $key => $data) {
            ...
            if (!$data['total'] || empty($data['summary_key']) || in_array($key, $skipped_keys)) {
                continue;
            }
            ...
            if (isset($data['settings_key']) && !empty($data['settings_key']) && ('instant_pay_amount' == $data['settings_key'])) {
                $cost = $data['data']['cost'];
            }
    ...
    {
        $results = [];
        $skipped_keys = ['deposit_free_total', 'extras_total', 'discount_total', 'total', 'total_hour'];
