...
$period_options      = array(
    'D' => __('Day(s)', 'wc-multivendor-membership'),
    'M' => __('Month(s)', 'wc-multivendor-membership'),
    'Y' => __('Year(s)', 'wc-multivendor-membership'),
);

$wcfm_membership_options  = get_option('wcfm_membership_options', array());
$membership_feature_lists = array();
if ( isset($wcfm_membership_options['membership_features']) ) {
    $membership_feature_lists = $wcfm_membership_options['membership_features'];
}

$membership_tax_settings = array();
if ( isset($wcfm_membership_options['membership_tax_settings']) ) {
    $membership_tax_settings = $wcfm_membership_options['membership_tax_settings'];
}
            ...
            <table>
              <tr class="invoice-number">
                ...
                <tr class="order-date">
                    ...
                    <td>
                      <?php
                        echo date_i18n(wc_date_format(), current_time('timestamp', 0));
                        ?>
                       @
                      <?php
                        echo date_i18n(wc_time_format(), current_time('timestamp', 0));
                        ?>
                    </td>
                </tr>
                    ...
                    <td>
                      <?php
                        if ( $is_free == 'yes' ) {
                            ...
                            if ( in_array($paymode, array( 'paypal_subs', 'paypal_subs_subs' )) ) {
                                $paymode = 'paypal';
                            }
                            if ( in_array($paymode, array( 'stripe', 'stripe_subs', 'stripe_subs_subs' )) ) {
                                $paymode = 'stripe';
                            }
                            if ( in_array($paymode, array( 'bank_transfer', 'bank_transfer_subs' )) ) {
                                ...
                                <td style="text-align:right;float:right;">
                                  <?php
                                    if ( ! $tax_percent ) {
                                        ...
                                        echo wc_price(wc_format_decimal($subscription_price * ( $tax_percent / 100 )));
                                    };
                                    ?>
                                </td>
                            </tr>
                                ...
                                if ( $is_free == 'yes' ) {
                                    ...
                                    if ( $subscription_type == 'one_time' ) {
                                        ...
                                        if ( $wcfm_register_member ) {
                                            ...
                                            $price_description = sprintf(__('%1$s for each %2$s %3$s', 'wc-multivendor-membership'), get_woocommerce_currency_symbol() . $billing_amt, $billing_period, $period_options[ $billing_period_type ]);
                                        } else {
                                            if ( ! empty($trial_period) && ! empty($trial_amt) ) {
                                                if ( $payment_method == 'stripe' ) {
                                                    echo wc_price(0);
                                                    $price_description  = sprintf(__('%1$s for each %2$s %3$s', 'wc-multivendor-membership'), get_woocommerce_currency_symbol() . $billing_amt, $billing_period, $period_options[ $billing_period_type ]);
                                                    $price_description .= ' ' . sprintf(__('with %1$s %2$s free trial', 'wc-multivendor-membership'), $trial_period, $period_options[ $trial_period_type ]);
                                                } else {
                                                    echo wc_price(wcfmvm_membership_tax_price($trial_amt));
                                                    $price_description  = ' ' . sprintf(__('%1$s for first %2$s %3$s', 'wc-multivendor-membership'), get_woocommerce_currency_symbol() . $trial_amt, $trial_period, $period_options[ $trial_period_type ]);
                                                    $price_description .= ' ' . sprintf(__('and then %1$s for each %2$s %3$s', 'wc-multivendor-membership'), get_woocommerce_currency_symbol() . $billing_amt, $billing_period, $period_options[ $billing_period_type ]);
                                                }
                                            } elseif ( ! empty($trial_period) && empty($trial_amt) ) {
                                                if ( $payment_method == 'paypal' ) {
                                                    echo wc_price(wcfmvm_membership_tax_price(1));
                                                    $price_description  = ' ' . sprintf(__('%1$s for first %2$s %3$s', 'wc-multivendor-membership'), get_woocommerce_currency_symbol() . 1, $trial_period, $period_options[ $trial_period_type ]);
                                                    $price_description .= ' ' . sprintf(__('and then %1$s for each %2$s %3$s', 'wc-multivendor-membership'), get_woocommerce_currency_symbol() . $billing_amt, $billing_period, $period_options[ $billing_period_type ]);
                                                } else {
                                                    echo wc_price(0);
                                                    $price_description  = sprintf(__('%1$s for each %2$s %3$s', 'wc-multivendor-membership'), get_woocommerce_currency_symbol() . $billing_amt, $billing_period, $period_options[ $billing_period_type ]);
