...
class WCFMu_Vendor_Invoice
{
    ...
    public function __construct()
    {
        ...
        if (! WCFM_Dependencies::wcfm_wc_pdf_invoices_packing_slips_plugin_active_check()) {
            return;
        }

        if ($WCFMu->is_marketplace) {
            // Invoice Settings
            add_action('end_wcfm_settings', [ &$this, 'wcfmu_vendor_invoice_settings' ], 16);
            add_action('wcfm_settings_update', [ &$this, 'wcfmu_vendor_invoice_settings_update' ], 16);

            add_action('end_wcfm_vendor_settings', [ &$this, 'wcfmu_vendor_invoice_store_settings' ], 16);
            add_action('end_wcfm_vendors_manage_form', [ &$this, 'wcfmmp_vendor_manage_invoice_setting' ], 13);
            add_action('wcfm_vendor_settings_update', [ &$this, 'wcfmu_vendor_invoice_store_settings_update' ], 16, 2);

            // Store Invoice in Customer Email attachment
            add_filter('woocommerce_email_attachments', [ $this, 'wcfmu_attach_vendor_pdf_to_email' ], 95, 3);

            // Membership Subscription Invoice
            add_filter('wcfm_membership_subscription_attachment', [ $this, 'wcfmu_membership_subscription_invoice' ], 95, 4);
        }
        ...
        if (apply_filters('wcfm_is_allow_pdf_invoice', true) || apply_filters('wcfm_is_allow_pdf_packing_slip', true) || apply_filters('wcfm_is_allow_store_invoice', true)) {
            add_filter('wcfm_orders_module_actions', [ &$this, 'wcfmu_pdf_invoice_orders_actions' ], 10, 4);
        }

        // WooCommerce PDF Invoices & Packing Slips Support
        add_action('wp_ajax_wcfm_order_pdf_invoice', [ &$this, 'wcfm_order_pdf_invoice' ]);
        add_action('wp_ajax_wcfm_order_pdf_packing_slip', [ &$this, 'wcfm_order_pdf_packing_slip' ]);

        // Store Payment Invoice
        add_action('wp_ajax_store_payment_invoice', [ &$this, 'wcfm_store_payment_invoice' ]);

        // WC My Account Order action
        add_filter('woocommerce_my_account_my_orders_actions', [ &$this, 'wcfm_my_account_store_order_pdf_invoice_download' ], 100, 2);
    ...
    }//end normalize_filter_args()


    function wcfmu_vendor_invoice_settings($wcfm_options)
    {
        global $WCFM, $WCFMu;
        $wcfm_vendor_invoice_options          = get_option('wcfm_vendor_invoice_options', []);
        ...
        $wcfm_vendor_invoice_fields           = isset($wcfm_vendor_invoice_options['fields']) ? $wcfm_vendor_invoice_options['fields'] : [];
        ?>
        <!-- collapsible -->
            ...
            <div id="wcfm_settings_form_vendor_invoice_expander" class="wcfm-content">
              <h2><?php echo apply_filters('wcfm_sold_by_label', '', __('Store', 'wc-frontend-manager')).' '.__('Store Invoice Setting', 'wc-frontend-manager-ultimate'); ?></h2>
                ...
                $WCFM->wcfm_fields->wcfm_generate_form_field(
                    apply_filters(
                        'wcfmu_settings_fields_vendor_invoice',
                        [
                            'wcfm_vendor_invoice_active'           => [
                                'label'       => __('Enable', 'wc-frontend-manager-ultimate'),
                                'type'        => 'checkbox',
                                'name'        => 'wcfm_vendor_invoice_options[enable]',
                                'class'       => 'wcfm-checkbox wcfm_ele',
                                'label_class' => 'wcfm_title checkbox_title wcfm_ele',
                                'value'       => 'yes',
                                'dfvalue'     => $wcfm_vendor_invoice_active,
                                'hints'       => __('Send out vendor store specific invoice to customer.', 'wc-frontend-manager-ultimate'),
                            ],
                            'wcfm_vendor_invoice_advance_font'     => [
                                'label'       => __('Advance Font', 'wc-frontend-manager-ultimate'),
                                'type'        => 'checkbox',
                                'name'        => 'wcfm_vendor_invoice_options[advance_font]',
                                'class'       => 'wcfm-checkbox wcfm_ele',
                                'label_class' => 'wcfm_title checkbox_title wcfm_ele',
                                'value'       => 'yes',
                                'dfvalue'     => $wcfm_vendor_invoice_advance_font,
                                'hints'       => __('First check with default font, if not working then only enable this.', 'wc-frontend-manager-ultimate'),
                                'desc_class'  => 'wcfm_page_options_desc',
                                // translators: 1) begin url 2) end url
                                'desc'        => sprintf(__('For having RTL support please install %1$sPDF Invoice mPDF library%2$s', 'wc-frontend-manager-ultimate'), '<a href="https://github.com/wpovernight/woocommerce-pdf-ips-mpdf/releases/download/v1.0.1/woocommerce-pdf-ips-mpdf.zip">', '</a>'),
                            ],
                            'wcfm_vendor_invoice_advance_currency' => [
                                'label'       => __('Advance Currency', 'wc-frontend-manager-ultimate'),
                                'type'        => 'checkbox',
                                'name'        => 'wcfm_vendor_invoice_options[advance_currency]',
                                'class'       => 'wcfm-checkbox wcfm_ele',
                                'label_class' => 'wcfm_title checkbox_title wcfm_ele',
                                'value'       => 'yes',
                                'dfvalue'     => $wcfm_vendor_invoice_advance_currency,
                                'hints'       => __('First check with default setting, if not working then only enable this.', 'wc-frontend-manager-ultimate'),
                            ],
                            'wcfm_vendor_invoice_logo'             => [
                                'label'       => __('Vendor Logo', 'wc-frontend-manager-ultimate'),
                                'type'        => 'checkbox',
                                'name'        => 'wcfm_vendor_invoice_options[logo]',
                                'class'       => 'wcfm-checkbox wcfm_ele',
                                'label_class' => 'wcfm_title checkbox_title wcfm_ele',
                                'value'       => 'yes',
                                'dfvalue'     => $wcfm_vendor_invoice_logo,
                            ],
                            'wcfm_vendor_invoice_store'            => [
                                'label'       => __('Vendor Store Name', 'wc-frontend-manager-ultimate'),
                                'type'        => 'checkbox',
                                'name'        => 'wcfm_vendor_invoice_options[store]',
                                'class'       => 'wcfm-checkbox wcfm_ele',
                                'label_class' => 'wcfm_title checkbox_title wcfm_ele',
                                'value'       => 'yes',
                                'dfvalue'     => $wcfm_vendor_invoice_store,
                            ],
                            'wcfm_vendor_invoice_address'          => [
                                'label'       => __('Vendor Address', 'wc-frontend-manager-ultimate'),
                                'type'        => 'checkbox',
                                'name'        => 'wcfm_vendor_invoice_options[address]',
                                'class'       => 'wcfm-checkbox wcfm_ele',
                                'label_class' => 'wcfm_title checkbox_title wcfm_ele',
                                'value'       => 'yes',
                                'dfvalue'     => $wcfm_vendor_invoice_address,
                            ],
                            'wcfm_vendor_invoice_email'            => [
                                'label'       => __('Vendor Email', 'wc-frontend-manager-ultimate'),
                                'type'        => 'checkbox',
                                'name'        => 'wcfm_vendor_invoice_options[email]',
                                'class'       => 'wcfm-checkbox wcfm_ele',
                                'label_class' => 'wcfm_title checkbox_title wcfm_ele',
                                'value'       => 'yes',
                                'dfvalue'     => $wcfm_vendor_invoice_email,
                            ],
                            'wcfm_vendor_invoice_phone'            => [
                                'label'       => __('Vendor Phone', 'wc-frontend-manager-ultimate'),
                                'type'        => 'checkbox',
                                'name'        => 'wcfm_vendor_invoice_options[phone]',
                                'class'       => 'wcfm-checkbox wcfm_ele',
                                'label_class' => 'wcfm_title checkbox_title wcfm_ele',
                                'value'       => 'yes',
                                'dfvalue'     => $wcfm_vendor_invoice_phone,
                            ],
                            'wcfm_vendor_invoice_policies'         => [
                                'label'       => __('Vendor policies', 'wc-frontend-manager-ultimate'),
                                'type'        => 'checkbox',
                                'name'        => 'wcfm_vendor_invoice_options[policies]',
                                'class'       => 'wcfm-checkbox wcfm_ele',
                                'label_class' => 'wcfm_title checkbox_title wcfm_ele',
                                'value'       => 'yes',
                                'dfvalue'     => $wcfm_vendor_invoice_policies,
                            ],
                            'wcfm_vendor_invoice_disclaimer'       => [
                                'label'       => __('Vendor Disclaimer', 'wc-frontend-manager-ultimate'),
                                'type'        => 'checkbox',
                                'name'        => 'wcfm_vendor_invoice_options[disclaimer]',
                                'class'       => 'wcfm-checkbox wcfm_ele',
                                'label_class' => 'wcfm_title checkbox_title wcfm_ele',
                                'value'       => 'yes',
                                'dfvalue'     => $wcfm_vendor_invoice_disclaimer,
                            ],
                            'wcfm_vendor_invoice_signature'        => [
                                'label'       => __('Vendor Signature', 'wc-frontend-manager-ultimate'),
                                'type'        => 'checkbox',
                                'name'        => 'wcfm_vendor_invoice_options[signature]',
                                'class'       => 'wcfm-checkbox wcfm_ele',
                                'label_class' => 'wcfm_title checkbox_title wcfm_ele',
                                'value'       => 'yes',
                                'dfvalue'     => $wcfm_vendor_invoice_signature,
                            ],
                            'wcfm_vendor_invoice_fields'           => [
                                'label'       => __('Additional Info', 'wc-frontend-manager-ultimate'),
                                'type'        => 'multiinput',
                                'name'        => 'wcfm_vendor_invoice_options[fields]',
                                'class'       => 'wcfm-text wcfm_ele',
                                'label_class' => 'wcfm_title',
                                'value'       => $wcfm_vendor_invoice_fields,
                                'options'     => [
                                    'is_active' => [
                                        'label'       => __('Enable', 'wc-frontend-manager-ultimate'),
                                        'type'        => 'checkbox',
                                    ...
                                    'field'     => [
                                        'label'       => __('Field', 'wc-frontend-manager-ultimate'),
                                        'type'        => 'text',
                                        'class'       => 'wcfm-text wcfm_ele',
    ...
    }//end wcfmu_vendor_invoice_settings()
    ...
    }//end wcfmu_vendor_invoice_settings_update()
        ...
        if ($wcfm_vendor_invoice_active && apply_filters('wcfm_is_allow_store_invoice', true)) {
            ...
            <div class="wcfm-container">
                <div id="wcfm_settings_form_vendor_invoice_expander" class="wcfm-content">
                    ...
                    $WCFM->wcfm_fields->wcfm_generate_form_field(
                        apply_filters(
                            'wcfmu_settings_fields_vendor_store_invoice',
                            [
                                'wcfm_vendor_invoice_prefix' => [
                                    'label'       => __('Invoice No Prefix', 'wc-frontend-manager-ultimate'),
                                    'type'        => 'text',
                                    'name'        => 'wcfm_vendor_invoice_options[prefix]',
                                    'class'       => 'wcfm-text wcfm_ele',
                                    'label_class' => 'wcfm_title wcfm_ele',
                                    'value'       => $wcfm_vendor_invoice_prefix,
                                ],
                                'wcfm_vendor_invoice_sufix'  => [
                                    'label'       => __('Invoice No. Sufix', 'wc-frontend-manager-ultimate'),
                                    'type'        => 'text',
                                    'name'        => 'wcfm_vendor_invoice_options[sufix]',
                                    'class'       => 'wcfm-text wcfm_ele',
                                    'label_class' => 'wcfm_title wcfm_ele',
                                    'value'       => $wcfm_vendor_invoice_sufix,
                                ],
                                'wcfm_vendor_invoice_digit'  => [
                                    'label'       => __('Invoice No. Digit', 'wc-frontend-manager-ultimate'),
                                    'type'        => 'number',
                                    'name'        => 'wcfm_vendor_invoice_options[digit]',
                                    'class'       => 'wcfm-text wcfm_ele',
                                    'label_class' => 'wcfm_title wcfm_ele',
                                    'value'       => $wcfm_vendor_invoice_digit,
                                    'hints'       => __('enter the number of digits here - enter `6` to display 42 as 000042', 'wc-frontend-manager-ultimate'),
                                    'attributes'  => [
                                        'min'  => 1,
                                        'step' => 1,
                                        'max'  => 9,
                    ...
                    if (! empty($wcfm_vendor_invoice_fields)) {
                        foreach ($wcfm_vendor_invoice_fields as $wvif_key => $wcfm_vendor_invoice_field) {
                            if (isset($wcfm_vendor_invoice_field['is_active']) && $wcfm_vendor_invoice_field['field']) {
                                $invoice_field_value = '';
                                if (isset($wcfm_vendor_invoice_data[$wvif_key]) && ! empty($wcfm_vendor_invoice_data[$wvif_key])) {
                                    $invoice_field_value = $wcfm_vendor_invoice_data[$wvif_key];
                                }

                                $WCFM->wcfm_fields->wcfm_generate_form_field(
                                    apply_filters(
                                        'wcfmu_settings_fields_vendor_store_invoice_custom',
                                        [
                                            'wvif_key_'.$wvif_key => [
                                                'label'       => $wcfm_vendor_invoice_field['field'],
                                                'name'        => 'wcfm_vendor_invoice_options['.$wvif_key.']',
                                                'type'        => 'text',
                                                'class'       => 'wcfm-text',
                                                'label_class' => 'wcfm_title',
                                                'value'       => $invoice_field_value,
                                            ],
                                        ],
                                        $vendor_id,
                                        $wvif_key,
                    ...
                    $WCFM->wcfm_fields->wcfm_generate_form_field(
                        apply_filters(
                            'wcfmu_settings_fields_vendor_store_invoice_more',
                            [
                                'wcfm_vendor_invoice_disclaimer' => [
                                    'label'       => __('Disclaimer', 'wc-frontend-manager-ultimate'),
                                    'type'        => 'textarea',
                                    'name'        => 'wcfm_vendor_invoice_options[disclaimer]',
                                    'class'       => 'wcfm-textarea wcfm_ele',
                                    'label_class' => 'wcfm_title wcfm_ele',
                                    'value'       => $wcfm_vendor_invoice_disclaimer,
                                ],
                                'wcfm_vendor_invoice_signature'  => [
                                    'label'       => __('Digital Signature', 'wc-frontend-manager-ultimate'),
                                    'type'        => 'upload',
                                    'name'        => 'wcfm_vendor_invoice_options[signature]',
                                    'class'       => 'wcfm-text wcfm_ele',
                                    'label_class' => 'wcfm_title wcfm_ele',
                                    'value'       => $wcfm_vendor_invoice_signature,
                                ],
                            ],
                            $vendor_id
                        )
    ...
    }//end wcfmu_vendor_invoice_store_settings()
    ...
    }//end wcfmmp_vendor_manage_invoice_setting()
    ...
    }//end wcfmu_vendor_invoice_store_settings_update()
    ...
    function wcfmu_pdf_invoice_orders_actions($actions, $order_id, $the_order, $vendor_id=0)
    {
        ...
        if (in_array($order_status, apply_filters('wcfm_pdf_invoice_download_disable_order_status', [ 'failed', 'cancelled', 'request', 'proposal', 'proposal-sent', 'proposal-expired', 'proposal-rejected', 'proposal-canceled', 'proposal-accepted' ]))) {
            return $actions;
        }
        ...
        if (wcfm_is_vendor() && apply_filters('wcfm_is_allow_store_invoice', true)) {
            $wcfm_store_invoices = (array) $the_order->get_meta('_wcfm_store_invoices');
    ...
    }//end wcfmu_pdf_invoice_orders_actions()
    ...
    public function wcfmu_attach_vendor_pdf_to_email($attachments, $email_id, $the_order)
    {
        ...
        if (! in_array($email_id, apply_filters('wcfm_allowed_store_invoice_order_emails', [ 'customer_invoice', 'new_order', 'customer_on_hold_order', 'customer_completed_order', 'customer_processing_order', 'store-new-order', 'new_renewal_order', 'customer_processing_renewal_order', 'customer_completed_renewal_order', 'customer_renewal_invoice' ])) || OrderUtil::get_order_type( $order_id ) !== 'shop_order') {
            return $attachments;
        }

        $document_type          = 'invoice';
        $wcfm_store_invoice_ids = $order->get_meta( '_wcfm_store_invoice_ids' );
        if (! $wcfm_store_invoice_ids) {
            $wcfm_store_invoice_ids = [];
        }

        $wcfm_store_invoices = $order->get_meta( '_wcfm_store_invoices' );
        if (! $wcfm_store_invoices) {
            $wcfm_store_invoices = [];
        } else {
        ...
        $pdf_filters = apply_filters('wpo_wcpdf_pdf_filters', []);
        ...
        $wcfm_store_invoice_items = [];
        $line_items               = $order->get_items('line_item');
        $line_items_shipping      = $order->get_items('shipping');
        $processed_item_vendor    = [];
        ...
        if (! empty($line_items)) {
            foreach ($line_items as $item_id => $item) {
                ...
                if (in_array($vendor_id, $processed_item_vendor)) {
                    continue;
                }

                // Store New Order vendor check
                if ($email_id == 'store-new-order') {
                    if ($WCFM->wcfm_marketplace && $WCFM->wcfm_marketplace->vendor_id && ( $WCFM->wcfm_marketplace->vendor_id != $vendor_id )) {
                        continue;
                    }
                }
        ...
        if (! empty($wcfm_store_invoice_items)) {
            foreach ($wcfm_store_invoice_items as $wcfm_store_invoice_item_key => $wcfm_store_invoice_item) {
                ...
                if (! $process_item_ids || ! is_array($process_item_ids) || empty($process_item_ids)) {
                    continue;
                }

                if (! $process_product_ids || ! is_array($process_product_ids) || empty($process_product_ids)) {
                    continue;
                }
                ...
                if ($wcfm_vendor_last_invoice_no) {
                    $wcfm_vendor_last_invoice_no = absint($wcfm_vendor_last_invoice_no);
                } else {
                    $wcfm_vendor_last_invoice_no = 0;
                }
                ...
                $filename = apply_filters('wcfmu_store_invoice_filename', __('invoice', 'wc-frontend-manager-ultimate').'-'.$wcfm_vendor_invoice_id.'.pdf', $wcfm_vendor_invoice_id, $vendor_id);
            ...
            $order->update_meta_data( '_wcfm_store_invoices', $wcfm_store_invoices );
            $order->update_meta_data( '_wcfm_store_invoice_ids', $wcfm_store_invoice_ids );
            $order->save();
        } else {
            return $attachments;
        }//end if
    ...
    }//end wcfmu_attach_vendor_pdf_to_email()
    ...
    function wcfmu_membership_subscription_invoice($attachments, $membership_id, $member_id, $is_register_member=false)
    {
        ...
        $filename = apply_filters('wcfmu_membership_invoice_filename', __('membership-invoice', 'wc-frontend-manager-ultimate').'-'.$membership_id.'.pdf', $membership_id, $vendor_id);
    ...
    }//end wcfmu_membership_subscription_invoice()
    ...
    function wcfm_order_pdf_invoice()
    {
        ...
        $filename = apply_filters('wcfmu_commission_invoice_filename', __('invoice', 'wc-frontend-manager-ultimate').'-'.$order_id.'.pdf', $order_id, $vendor_id);

        do_action('wpo_wcpdf_created_manually', $pdf, $filename);

        // Get output setting
        $output_mode = 'download';
        // isset($general_settings['download_display']) ? $general_settings['download_display'] : '';
    ...
    function wcfm_order_pdf_packing_slip()
    {
        ...
        $filename = apply_filters('wcfmu_packing_slip_filename', __('packing-slip', 'wc-frontend-manager-ultimate').'-'.$order_id.'.pdf', $order_id);
    ...
    }//end wcfm_order_pdf_packing_slip()
    ...
    function wcfm_store_payment_invoice()
    {
        ...
        $filename = apply_filters('wcfmu_withdrawal_invoice_filename', __('withdrawal-invoice', 'wc-frontend-manager-ultimate').'-'.sprintf('%06u', $withdrawal_id).'.pdf', $withdrawal_id);
    ...
    }//end wcfm_store_payment_invoice()
    ...
    function wcfm_my_account_store_order_pdf_invoice_download($actions, $order)
    {
        ...
        if (! apply_filters('wcfm_is_allow_my_account_store_order_pdf_invoice_download', true)) {
            return $actions;
        }
            ...
            foreach ($wcfm_store_invoices as $vendor_id => $wcfm_store_invoice) {
                if (file_exists($wcfm_store_invoice)) {
                    if (empty($upload_dir['error'])) {
                        ...
                        $actions['wcfm-store-invoice-'.$vendor_id] = [
                            'name' => apply_filters('wcfm_store_invoice_download_label', wcfm_get_vendor_store_name(absint($vendor_id)).' '.$sold_by_text.' '.__('Invoice', 'wc-frontend-manager-ultimate'), $order->get_id(), $vendor_id),
                            'url'  => $invoice_path,
                        ];
                    }
                }
    ...
    }//end wcfm_my_account_store_order_pdf_invoice_download()
